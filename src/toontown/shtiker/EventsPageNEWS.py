...
class EventsPage(ShtikerPage.ShtikerPage):
    ...
    def getActivityItem(self, activityBase, count = 1):
        ...
        if count == 1:
            ...
            textForActivity = '{0} x {1:d}'.format(activityName, count)
    ...
    def getDecorationItem(self, decorBase, count = 1):
        ...
        label = DirectLabel(relief=None, geom=self.decorationModels.find('**/partyDecoration_{0}'.format(assetName)), text=textForDecoration, text_scale=TTLocalizer.EPdecorationItemLabel, text_align=TextNode.ACenter, text_pos=(-0.01, -0.43), text_wordwrap=7.0)
    ...
    def loadInvitations(self):
        ...
        for partyInfo in base.localAvatar.partiesInvitedTo:
            ...
            if inviteInfo is None:
                EventsPage.notify.error('No invitation info for party id {0:d}'.format(partyInfo.partyId))
                return
    ...
    def createListAndLabel(self, parent, gui, typeString, itemsVisible, downString = 'DownArrow', upString = 'UpArrow', textString = '_text_locator'):
        list = DirectScrolledList(parent=parent, relief=None, incButton_image=(gui.find('**/{0}{1}_up'.format(typeString, downString)),
         gui.find('**/{0}{1}_down'.format(typeString, downString)),
         gui.find('**/{0}{1}_rollover'.format(typeString, downString)),
         gui.find('**/{0}{1}_inactive'.format(typeString, downString))), incButton_relief=None, decButton_image=(gui.find('**/{0}{1}_up'.format(typeString, upString)),
         gui.find('**/{0}{1}_down'.format(typeString, upString)),
         gui.find('**/{0}{1}_rollover'.format(typeString, upString)),
         gui.find('**/{0}{1}_inactive'.format(typeString, upString))), decButton_relief=None, itemFrame_pos=gui.find('**/{0}_locator'.format(typeString)).getPos(), itemFrame_relief=None, numItemsVisible=itemsVisible, forceHeight=0.07)
        ...
        label = DirectLabel(parent=parent, relief=None, text=strings[typeString], text_scale=TTLocalizer.EPcreateListAndLabel, pos=gui.find('**/{0}{1}'.format(typeString, textString)).getPos())
        return (list, label)
    ...
    def changePartyPrivateResponseReceived(self, partyId, newPrivateStatus, errorCode):
        EventsPage.notify.debug('changePartyPrivateResponseReceived called with partyId = {0:d}, newPrivateStatus = {1:d}, errorCode = {2:d}'.format(partyId, newPrivateStatus, errorCode))
        taskMgr.remove('changePartyPrivateResponseReceivedTimeOut')
        self.ignore('changePartyPrivateResponseReceived')
    ...
    def __doCancelParty(self):
        if self.hostedPartyInfo and self.hostedPartyInfo.status == PartyGlobals.PartyStatus.Pending or self.hostedPartyInfo.status == PartyGlobals.PartyStatus.CanStart or self.hostedPartyInfo.status == PartyGlobals.PartyStatus.NeverStarted:
            self.hostingCancelButton['state'] = DirectGuiGlobals.DISABLED
    ...
    def changePartyStatusResponseReceived(self, partyId, newPartyStatus, errorCode, beansRefunded):
        EventsPage.notify.debug('changePartyStatusResponseReceived called with partyId = {0:d}, newPartyStatus = {1:d}, errorCode = {2:d}'.format(partyId, newPartyStatus, errorCode))
        if errorCode == PartyGlobals.ChangePartyFieldErrorCode.AllOk and newPartyStatus == PartyGlobals.PartyStatus.Cancelled:
            self.loadHostedPartyInfo()
    ...
    def createArticleTextList(self):
        ...
        self.notify.debug('bottomLeft={0} topRight={1}'.format(bottomLeft.getPos(), topRight.getPos()))
        buttonOffSet = 0.045
        selectedIndex = 0
        ...
        self.notify.debug('topLeft={0} topLeftStart={1}'.format(topLeft.getPos(), topLeftStart))
        ...
        self.notify.debug('scrollTopUp pos wrt topLeft {0}'.format(decButtonPos))
        self.notify.debug('scrollTopUp pos normal {0}'.format(scrollTopUp.getPos()))
    ...
    def downloadArticles(self):
        if self.gotArticles:
            return
        if not self.NonblockingDownload and not taskMgr.hasTaskNamed(self.DownloadArticlesTaskName):
    ...
    def downloadArticlesTask(self, task):
        ...
        try:
            ...
            self.notify.warning('Could not open {0}'.format(self.getNewsUrl()))
        ...
        for index in xrange(len(urls) / 2):
            ...
            try:
                self.notify.info('opening {0}'.format(imageUrl))
                imageFile = urllib.urlopen(imageUrl)
                data = imageFile.read()
                img.read(StringStream(data))
                imageFile.close()
            except IOError:
                self.notify.warning('image url {0:d} could not open {1:d}'.format(index, imageUrl))

            text = ''
            self.articleText[index] = text
            try:
                self.notify.info('opening {0}'.format(textUrl))
                ...
                self.notify.warning('text url {0:d} could not open {1}'.format(index, textUrl))
    ...
    def displayArticleImage(self, img):
        ...
        shrinkY = True
        if imgAspectRatio > maxAspectRatio and xSize:
    ...
    def doneGettingUrl(self, url, data, allOk):
        self.notify.debug('doneGettingUrl {0} {1} {2}'.format(url, type(data), allOk))
        self.printCurFields()
        if url == self.getNewsUrl():
            if allOk:
                ...
                self.notify.warning('Could not open {0}'.format(url))
                self.newsStatusLabel['text'] = TTLocalizer.EventsPageNewsUnavailable
                self.gotArticles = True
                return
            if self.urls:
            ...
            if self.curDownloadIsJpg:
                ...
                try:
                    if type(data) == Ramfile and allOk:
                        img.read(StringStream(data.getData()))
                    else:
                        img.read(StringStream(data))
                except:
                    self.notify.warning('image url {0:d} could not read {1}'.format(self.curArticleIndex, url))
    ...
    def printCurFields(self):
        self.notify.debug('curUrlIndex={0} curArticleIndex={1} curDownloadIsJpg={2}'.format(self.curUrlIndex, self.curArticleIndex, self.curDownloadIsJpg))
    ...
    def getFileUrl(self, fileUrl):
        ...
        try:
            ...
            self.notify.warning('Could not open {0}'.format(fileUrl))
            result = False
    ...
    def pollDownloadTask(self, task):
        ...
        if result == 0:
            ...
            if self.nonBlock.getStatusString() == 'OK':
                ...
                self.notify.warning('{0} for {1}'.format(self.nonBlock.getStatusString(), task.url))
