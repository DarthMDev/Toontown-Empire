...
import SuitTimings
from otp.ai.AIBaseGlobal import config
...
from toontown.dna.SuitLegList import SuitLeg
...
class DistributedSuitPlannerAI(DistributedObjectAI.DistributedObjectAI,
        ...
        if simbase.air.wantCogdominiums:
            if not hasattr(self.__class__, 'CogdoPopAdjusted'):
                ...
                for index in xrange(len(self.SuitHoodInfo)):
                    SuitBuildingGlobals.buildingMinMax[self.zoneId][0] = int(0.5 +
                                                                             self.CogdoPopFactor *
                                                                             SuitBuildingGlobals.
                                                                             buildingMinMax
                                                                             [self.zoneId][0])
                    SuitBuildingGlobals.buildingMinMax[self.zoneId][1] = int(0.5 +
                                                                             self.CogdoPopFactor *
                                                                             SuitBuildingGlobals
                                                                             .buildingMinMax
                                                                             [self.zoneId][1]
    ...
    def initBuildingsAndPoints(self):
        ...
        for p in self.frontdoorPointList:
            ...
            if blockNumber in self.buildingFrontDoors:
                self.notify.debug('Multiple front doors for building %s in zone %s' % (blockNumber,
                                                                                       self.zoneId)
                                  )
                continue
        ...
        for p in self.sidedoorPointList:
            blockNumber = p.getLandmarkBuildingIndex()
            if blockNumber < 0:
                self.notify.debug('No landmark building for (%s) in zone %s' % (str(p),
                                                                                self.zoneId))
                continue
        ...
        for bldg in self.buildingMgr.getBuildings():
            ...
            if blockNumber not in self.buildingFrontDoors:
                self.notify.debug('No front door for building %s in zone %s' % (blockNumber,
                                                                                self.zoneId))
            if blockNumber not in self.buildingSideDoors:
                self.notify.debug('No side door for building %s in zone %s' % (blockNumber,
                                                                               self.zoneId))
    ...
    def createNewSuit(self, blockNumbers, streetPoints, toonBlockTakeover=None,
        ...
        blockNumber = None
        if self.notify.getDebug():
            self.notify.debug('Choosing origin from %d+%d possibles.' % (len(streetPoints),
                                                                         len(blockNumbers)))
            ...
            if bn in self.buildingSideDoors:
                for doorPoint in self.buildingSideDoors[bn]:
                    ...
                    while blockNumber is None and i >= 0:
                        ...
                        startTime += self.dnaStore.getSuitEdgeTravelTime(doorPoint.getIndex(),
                                                                         pi, self.suitWalkSpeed)
        ...
        if (suitLevel is None) and (buildingHeight is not None):
            suitLevel = self.chooseSuitLevel(self.SuitHoodInfo[self.hoodInfoIdx]
                                             [self.SUIT_HOOD_INFO_LVL], buildingHeight)
        ...
        gotDestination = self.chooseDestination(newSuit, startTime,
                                                ...
                                                cogdoTakeover=cogdoTakeover, minPathLen=minPathLen,
    ...
    def newSuitShouldAttemptTakeover(self):
        ...
        if self.numAttemptingTakeover >= numNeeded:
            self.pendingBuildingTracks = []
            return False
        self.notify.debug('DSP %s is planning a takeover attempt in zone %s' % (self.getDoId(),
                                                                                self.zoneId))
        return True
    ...
    def chooseDestination(self, suit, startTime, toonBlockTakeover=None,
        ...
        if self.notify.getDebug():
            self.notify.debug('Choosing destination point from %s+%s possibles.' % (len(possibles),
                              len(backup)))
        if len(possibles) == 0:
            possibles = backup
            backup = []
        ...
        if self.notify.getDebug() and self.currDesired is None:
            self.notify.debug('zone %d has %d of %d fly-in and %d of %d building suits.'
                              % (self.
                                 zoneId,
                                 self.
                                 numFlyInSuits, targetFlyInNum, self.numBuildingSuits,
                                 targetBuildingNum))
        ...
        if self.buildingMgr:
            ...
            if timeout is not None:
                ...
                if oldestAge > timeout:
                    self.notify.info('''Street % d has % d buildings
                                     reclaiming %0.2f-hour-old building
        ...
        if suitBlockCount < self.targetNumSuitBuildings:
            for _ in xrange(self.targetNumSuitBuildings - suitBlockCount):
                ...
                if suitName is not None:
                    suitType = SuitDNA.getSuitType(suitName)
                    suitTrack = SuitDNA.getSuitDept(suitName)
                (suitLevel, suitType, suitTrack) = self.pickLevelTypeAndTrack(None, suitType,
                                                                              suitTrack
                                                                              )
    ...
    def assignInitialSuitBuildings(self):
        ...
        self.notify.debug('''Want % s out of %s total suit buildings; we currently have %s assigned, %s
                          actual.''' % (wantedSuitBuildings, totalBuildings, targetSuitBuildings,
                                        actualSuitBuildings))
        if actualSuitBuildings > 0:
            ...
            for sp in self.air.suitPlanners.values():
                ...
                if numBuildings > sp.targetNumSuitBuildings:
                    ...
                    numReassigned += more
            if numReassigned > 0:
                self.notify.debug('Assigned %s buildings where suit buildings already existed.' %
                                  numReassigned
                                  )
    ...
    def assignSuitBuildings(self, numToAssign):
        ...
        while numToAssign > 0:
            ...
            if not smallestTracks:
                self.notify.info('No more room for buildings, with %s still to assign.' %
                                 numToAssign)
                return
            ...
            for height in xrange(5):
                if totalWeightPerHeight[height]:
                    count = float(numPerHeight[height]) / float(self.BUILDING_HEIGHT_DISTRIBUTION
                                                                [height]
                                                                )
                    if (smallestCount is None) or (count < smallestCount):
            ...
            while repeat and (buildingTrack is not None) and (buildingHeight is not None):
                if len(hoodInfo) == 0:
                    self.notify.warning('''No more streets can have suit buildings, with %s buildings
                                        unassigned!''' % numToAssign)
                    return
                repeat = 0
                currHoodInfo = self.chooseStreetWithPreference(hoodInfo, buildingTrackIndex,
                                                               buildingHeight)
                ...
                if zoneId in self.air.suitPlanners:
                    ...
                    numTarget = 0
                    numTotalBuildings = 0
                if ((numTarget >= SuitBuildingGlobals.buildingMinMax[self.zoneId]
                     [1]) or (numTarget >= numTotalBuildings)):
            ...
            if (buildingTrack is not None) and (buildingHeight is not None):
                ...
                self.notify.info('''Assigning building to zone %s, pending tracks = %s,
                                 pending heights = %s''' % (zoneId, sp.pendingBuildingTracks,
    ...
    def unassignSuitBuildings(self, numToAssign):
        ...
        while numToAssign > 0:
            ...
            while repeat:
                if len(hoodInfo) == 0:
                    self.notify.warning('''No more streets can remove suit buildings, with % s buildings
                                        too many!''' % numToAssign)
                    return
                repeat = 0
    ...
    def requestBattle(self, zoneId, suit, toonId):
        self.notify.debug('requestBattle() - zone: %s suit: %s toon: %s' % (zoneId, suit.doId,
                                                                            toonId))
        ...
        for currOther in self.zoneInfo[zoneId]:
            self.notify.debug('Found suit %s in this new battle zone %s' % (currOther.getDoId(),
                                                                            zoneId))
            if currOther != suit:
    ...
    def checkForBattle(self, zoneId, suit):
        if self.battleMgr.cellHasBattle(zoneId):
            if self.__suitCanJoinBattle(zoneId) and self.battleMgr.requestBattleAddSuit(zoneId,
                                                                                        suit):
                return 1
            suit.flyAwayNow()
            return 1
    ...
    def postBattleResumeCheck(self, suit):
        self.notify.debug('DistSuitPlannerAI:postBattleResumeCheck: suit ' + str(suit.getDoId()) +
                          ' is leaving battle')
        battleIndex = 0
