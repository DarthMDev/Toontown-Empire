...
class DistributedTrolley(DistributedObject.DistributedObject):
    ...
    def __init__(self, cr):
        ...
        self.trolleyCountdownTime = base.config.GetFloat('trolley-countdown-time',
    ...
    def emptySlot(self, index, avId, timestamp):
        if avId == 0:
            ...
            toon.stopSmooth()
            sitStartDuration = toon.getDuration('sit-start')
            track = Sequence(Parallel(ActorInterval(toon, 'sit-start', startTime=sitStartDuration,
                             endTime=0.0), Sequence(Wait(sitStartDuration * 0.5),
                             LerpPosInterval(toon, sitStartDuration * 0.25, Point3(-5,
                                                                                   -4.5 + index * 3,
                                                                                   1.4
                             ...
                             LerpPosInterval(toon,
                                             TOON_EXIT_TIME, Point3(21 - index * 3,
                                                                    -5, 0.02),
                             other=self.trolleyStation), Func(self.notifyToonOffTrolley, toon), Func(self.clearToonTrack, avId), name=toon.uniqueName('emptyTrolley'), autoPause=1)
